...
// Combined schema covering all env vars tested (matches the old flat schema)
const fullEnvSchema = commonEnvSchema.extend({
...
describe('parseEnv', () => {
  ...
  it('respects overridden defaults', () => {
    const result = parseEnv(fullEnvSchema, {
      ...validEnv,
      PORT: '4000',
      LOG_LEVEL: 'debug',
  ...
  it('throws when DATABASE_URL is missing', () => {
    ...
    expect(() => parseEnv(fullEnvSchema, rest)).toThrow('Invalid environment variables');
  });
  ...
  it('throws on invalid URL format', () => {
    expect(() =>
      parseEnv(fullEnvSchema, { ...validEnv, DATABASE_URL: 'not-a-url' }),
    ).toThrow('Invalid environment variables');
  });

  it('throws on invalid LOG_LEVEL value', () => {
    expect(() =>
      parseEnv(fullEnvSchema, { ...validEnv, LOG_LEVEL: 'verbose' }),
    ).toThrow('Invalid environment variables');
  });
});
